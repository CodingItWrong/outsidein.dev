...
In `src`, create a `components` folder, then inside it create a `RestaurantScreen.js` file. For the moment let's add just enough content to make it a valid component. Add the following:
...
Then, in `App.js`, add this component:
...
We import the `RestaurantScreen` component so `App` has access to it, then we add it to the returned JSX.
